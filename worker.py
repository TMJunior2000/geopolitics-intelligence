...
def fetch_youtube_videos(channel_url) -> List[dict]:
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl: # type: ignore
        try:
            ...
            log(f"Errore nel fetch YouTube: {e}")
    return videos
...
def analyze_with_gemini(text: str) -> dict:
    if not text or len(text) < 100:
        return {"summary": "Contenuto insufficiente per analisi", "risk_level": "LOW"}

    prompt = """
    Sei un analista geopolitico esperto. Analizza la trascrizione e restituisci SOLO JSON:
    {
    ...
    try:
        ...
        return {"summary": "Errore analisi AI"}
